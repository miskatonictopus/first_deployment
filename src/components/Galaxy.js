...
for (int y = -1; y <= 1; y++) {
  for (int x = -1; x <= 1; x++) {
    ...
    // decide aleatoriamente si será azul o blanca
    float choice = Hash21(si + 7.0);
    vec3 base;
    if (choice < 0.2) {
      // 20% azules
      base = vec3(0.4, 0.6, 1.0) * val;
    } else {
      // 80% blancas
      base = vec3(val);
    }
...
export default function Galaxy({ focal = [0.5, 0.5], rotation = [1.0, 0.0], starSpeed = 0.5, density = 1, hueShift = 140, disableAnimation = false, speed = 1.0, mouseInteraction = true, glowIntensity = 0.3, saturation = 0.0, mouseRepulsion = true, repulsionStrength = 2, twinkleIntensity = 0.3, rotationSpeed = 0.1, autoCenterRepulsion = 0, transparent = true, ...rest }) {
    ...
    useEffect(() => {
        ...
        if (transparent) {
            ...
            gl.clearColor(0, 0, 0, 0); // <- transparente
        }
        else {
            gl.clearColor(0, 0, 0, 1); // <- aquí puedes cambiar el color del fondo
        }
        let program;
        function resize() {
            const scale = 1;
    ...
    return _jsx("div", { ref: ctnDom, className: "galaxy-container", ...rest });
}
